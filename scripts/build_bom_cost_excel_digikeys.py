"""Build a BOM cost Excel workbook with DigiKey pricing.

Usage:
  uv run --with openpyxl python scripts/build_bom_cost_excel_digikeys.py <bom.csv> [output.xlsx]

DigiKey part data is fetched only through the KiCad MCP server (default: http://127.0.0.1:8500/mcp).
Start the MCP server with DigiKey OAuth credentials configured in that process before running this script.
This script never reads DigiKey credentials from its own terminal environment.
"""
...
from kicad_mcp.library.providers.digikey import MAX_PART_NUMBERS
...
def excel_tiered_unit_price_formula(order_qty_expr: str, breaks: list[tuple[int, float]]) -> str:
    ...
    # IFERROR: order qty below the lowest DigiKey break otherwise yields MATCH #N/A.
    return (
...
def record_to_lookup_entry(record) -> dict[str, object]:
    ...
    return {
        ...
        "digikey_pn": record.distributor_part_number,
...
def is_digikey_in_stock(info: dict[str, object]) -> bool:
...
async def mcp_search_part(
    ...
    *,
    ...
    attempts: list[dict[str, str]] = [{"part_number": mpn, "provider": "digikey", "match_mode": "Exact"}]
    ...
    result = await session.call_tool(
        ...
        {"keyword": mpn, "provider": "digikey", "records": 10},
    )
    payload = tool_json(result)
...
async def probe_mcp_digikey(mcp_url: str) -> bool:
...
async def lookup_digikey_prices_mcp(
    rows: list[BomRow],
    mcp_url: str,
        ...
        async with ClientSession(read_stream, write_stream) as session:
            ...
            for start in range(0, len(mpns), MAX_PART_NUMBERS):
                ...
                payload = await mcp_call_part_search(
                    session,
                    {"part_number": query, "provider": "digikey", "match_mode": "Exact"},
                    tool_json,
                )
                if payload.get("error"):
...
def _dict_to_record(item: dict):
    ...
    return ComponentRecord(
        provider=str(item.get("provider", "digikey")),
...
def lookup_digikey_prices(rows: list[BomRow], mcp_url: str | None) -> dict[str, dict[str, object]]:
    resolved_mcp_url = (mcp_url or DEFAULT_MCP_URL).strip()

    if not asyncio.run(probe_mcp_digikey(resolved_mcp_url)):
        raise SystemExit(
            "DigiKey is not available via MCP.\n"
            "  Start the KiCad MCP server with DigiKey OAuth credentials configured, then retry.\n"
            f"  MCP URL: {resolved_mcp_url}"
        )

    print(f"Using MCP server for DigiKey lookups: {resolved_mcp_url}")
    return asyncio.run(lookup_digikey_prices_mcp(rows, resolved_mcp_url))
...
def build_workbook(
    ...
    for row, info in available:
        ...
        ws.cell(row=current_row, column=10, value=str(info.get("digikey_pn", "")))
    ...
    if unavailable:
        ...
        non_digikey_total_row = current_row
        current_row += 2
    else:
        current_row += 1
    ...
    digikey_instock_summary_row = current_row
    current_row += 1

    ws.cell(row=current_row, column=5, value="Total — DigiKey (outstock)").font = Font(bold=True)
    ws.cell(row=current_row, column=9, value=f"={digikey_outstock_total_ref}").number_format = "$0.00"
    ...
    combined_total_cell = ws.cell(
        ...
        column=9,
        value=(
            f"=I{digikey_instock_summary_row}+I{digikey_outstock_summary_row}+I{non_digikey_summary_row}"
        ),
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help="Output Excel path (default: <bom>_bom_cost_digikeys.xlsx beside CSV)",
    )
    parser.add_argument(
        "--mcp-url",
    ...
    output_path = (
        ...
        if args.output_xlsx
        else bom_path.with_name(f"{bom_path.stem}_bom_cost_digikeys.xlsx")
    )
    ...
    lookup = lookup_digikey_prices(bom_rows, args.mcp_url.strip())
    ...
    print(f"DigiKey matches: {available_count}/{len(placed_rows)} placed lines")
    return 0

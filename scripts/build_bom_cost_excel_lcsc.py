"""Build a BOM cost Excel workbook with LCSC pricing.

Usage:
  uv run --with openpyxl python scripts/build_bom_cost_excel_lcsc.py <bom.csv> [output.xlsx]

LCSC part data is fetched only through the KiCad MCP server (default: http://127.0.0.1:8500/mcp).
Start the MCP server before running this script. LCSC requires no API key.
"""
...
from kicad_mcp.library.providers.lcsc import MAX_PART_NUMBERS
...
def excel_tiered_unit_price_formula(order_qty_expr: str, breaks: list[tuple[int, float]]) -> str:
    ...
    # IFERROR: order qty below the lowest LCSC break otherwise yields MATCH #N/A.
    return (
...
def record_to_lookup_entry(record) -> dict[str, object]:
    ...
    return {
        ...
        "lcsc_pn": record.distributor_part_number,
...
def is_lcsc_in_stock(info: dict[str, object]) -> bool:
...
async def mcp_search_part(
    ...
    *,
    ...
    attempts: list[dict[str, str]] = [{"part_number": mpn, "provider": "lcsc", "match_mode": "Exact"}]
    ...
    result = await session.call_tool(
        ...
        {"keyword": mpn, "provider": "lcsc", "records": 10},
    )
    payload = tool_json(result)
...
async def probe_mcp_lcsc(mcp_url: str) -> bool:
...
async def lookup_lcsc_prices_mcp(
    rows: list[BomRow],
    mcp_url: str,
        ...
        async with ClientSession(read_stream, write_stream) as session:
            ...
            for start in range(0, len(mpns), MAX_PART_NUMBERS):
                ...
                payload = await mcp_call_part_search(
                    session,
                    {"part_number": query, "provider": "lcsc", "match_mode": "Exact"},
                    tool_json,
                )
                if payload.get("error"):
...
def _dict_to_record(item: dict):
    ...
    return ComponentRecord(
        provider=str(item.get("provider", "lcsc")),
...
def lookup_lcsc_prices(rows: list[BomRow], mcp_url: str | None) -> dict[str, dict[str, object]]:
    resolved_mcp_url = (mcp_url or DEFAULT_MCP_URL).strip()

    if not asyncio.run(probe_mcp_lcsc(resolved_mcp_url)):
        raise SystemExit(
            "LCSC is not available via MCP.\n"
            f"  Start the KiCad MCP server, then retry.\n"
            f"  MCP URL: {resolved_mcp_url}"
        )

    print(f"Using MCP server for LCSC lookups: {resolved_mcp_url}")
    return asyncio.run(lookup_lcsc_prices_mcp(rows, resolved_mcp_url))
...
def build_workbook(
    ...
    non_lcsc_total_row: int | None = None
    if unavailable:
        ws.cell(row=current_row, column=5, value="Total (Not on LCSC × PCBA Qty)").font = Font(bold=True)
        non_lcsc_total_cell = ws.cell(
            row=current_row,
            column=8,
            value=f"=SUM(H{unavailable_first_data_row}:H{unavailable_last_data_row})*{pcba_cell}",
        )
        non_lcsc_total_cell.font = Font(bold=True)
        non_lcsc_total_cell.number_format = "$0.00"
        non_lcsc_total_row = current_row
        current_row += 2
    else:
        current_row += 1
    ...
    current_row += 1
    lcsc_instock_total_ref = f"I{lcsc_instock_total_row}" if lcsc_instock_total_row else "0"
    lcsc_outstock_total_ref = f"I{lcsc_outstock_total_row}" if lcsc_outstock_total_row else "0"
    non_lcsc_total_ref = f"H{non_lcsc_total_row}" if non_lcsc_total_row else "0"

    ws.cell(row=current_row, column=5, value="Total — LCSC (instock)").font = Font(bold=True)
    ws.cell(row=current_row, column=9, value=f"={lcsc_instock_total_ref}").number_format = "$0.00"
    lcsc_instock_summary_row = current_row
    current_row += 1

    ws.cell(row=current_row, column=5, value="Total — LCSC (outstock)").font = Font(bold=True)
    ws.cell(row=current_row, column=9, value=f"={lcsc_outstock_total_ref}").number_format = "$0.00"
    lcsc_outstock_summary_row = current_row
    current_row += 1

    ws.cell(row=current_row, column=5, value="Total — Not on LCSC (× PCBA Qty)").font = Font(bold=True)
    ws.cell(row=current_row, column=9, value=f"={non_lcsc_total_ref}").number_format = "$0.00"
    non_lcsc_summary_row = current_row
    current_row += 1
    ...
    combined_total_cell = ws.cell(
        ...
        column=9,
        value=(
            f"=I{lcsc_instock_summary_row}+I{lcsc_outstock_summary_row}+I{non_lcsc_summary_row}"
        ),
    )
...
def main() -> int:
    ...
    output_path = (
        ...
        if args.output_xlsx
        else bom_path.with_name(f"{bom_path.stem}_bom_cost_lcsc.xlsx")
    )
    ...
    lookup = lookup_lcsc_prices(bom_rows, args.mcp_url.strip())
    ...
    print(f"LCSC matches: {available_count}/{len(placed_rows)} placed lines")
    return 0
